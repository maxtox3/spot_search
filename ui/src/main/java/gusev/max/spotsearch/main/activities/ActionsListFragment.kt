...
class ActionsListFragment :
    ...
    override fun render(state: ActionsViewState) {
        when {
            ...
            state is ActionsViewState.BrowseActionsSuccess -> setupForSuccess(state.actions)
        }
    }

    private fun setupForSuccess(actions: List<ActionModel>?) {
        getLoadingView().visibility = View.GONE
        if (actions == null || actions.isEmpty()) {
            setupScreenForError(resources.getString(R.string.error_empty))
        } else {
            actionsAdapter.setData(actions)
        }
    }
